...
\vspace{1em}
A GNFA is a modified NFA where the transitions are labeled by \textbf{entire regular expressions}, not just single symbols or $\eps$.
...
%------------------------------------------------
\begin{frame}{The GNFA Conversion Algorithm}

We can formalize this state-ripping process as a recursive algorithm.

\vspace{0.8em}
\textbf{Algorithm} $\text{\textsc{Convert}}(G)$:
\begin{enumerate}
    \item Let $k$ be the number of states of the GNFA $G$.
    \item \textbf{If $k = 2$:}
    \begin{itemize}
        \item The two states must be $q_{start}$ and $q_{accept}$. 
        \item \textbf{Return} the regular expression labeling the transition between them: $R(q_{start}, q_{accept})$.
    \end{itemize}
    \item \textbf{If $k > 2$:}
    \begin{itemize}
        \item Select any state $q_{rip} \in Q \setminus \{q_{start}, q_{accept}\}$.
        \item Construct a new GNFA $G'$ with $k-1$ states by removing $q_{rip}$.
        \item For every pair of states $q_i \in Q' \setminus \{q_{accept}\}$ and $q_j \in Q' \setminus \{q_{start}\}$:
        \[
        R_{new}(q_i, q_j) = R(q_i, q_j) \cup \left( R(q_i, q_{rip}) (R(q_{rip}, q_{rip}))^* R(q_{rip}, q_j) \right)
        \]
        \item \textbf{Return} $\text{\textsc{Convert}}(G')$.
    \end{itemize}
\end{enumerate}
...
%------------------------------------------------
\begin{frame}{Example: Binary Divisible by 3}
\textbf{Goal:} Convert the DFA for ``$\eps$ or binary strings divisible by 3'' into a Regular Expression.

\vspace{0.5em}
\textbf{Step 0: The Original DFA}
\begin{center}
\begin{tikzpicture}[
  ->,>=Stealth,shorten >=1pt,
  node distance=2.5cm,
...
\end{center}
\vspace{0.5em}
\small \textit{Note:} States represent the value modulo 3. Since $\eps$ evaluates to 0, $q_0$ is both the start and accept state.
\end{frame}

%------------------------------------------------
\begin{frame}{Step 1: Convert to GNFA}
Add a new start state $q_s$ and a new accept state $q_a$. Connect them with $\eps$-transitions.
...
\node[state, right of=qs] (q0) {$q_0$};
\node[state, right of=q0] (q1) {$q_1$};
\node[state, right of=q1] (q2) {$q_2$};
\node[state, accepting, below=1cm of q0] (qa) {$q_a$};

\path 
(qs) edge node[above] {$\eps$} (q0)
(q0) edge node[left] {$\eps$} (qa)
(q0) edge[loop above] node {$0$} ()
(q0) edge[bend left] node[above] {$1$} (q1)
(q1) edge[bend left] node[below] {$1$} (q0)
(q1) edge[bend left] node[above] {$0$} (q2)
...
%------------------------------------------------
\begin{frame}{Step 2: Rip $q_2$}
When we remove $q_2$, we must fix any paths that went \textit{through} $q_2$.
The only such path is $q_1 \to q_2 \to q_1$.

\vspace{0.5em}
\textbf{New $q_1 \to q_1$ transition:}
\[
R_{new}(q_1,q_1) = R_{old}(q_1,q_1) \cup (0 \circ 1^* \circ 0) = \emptyset \cup 01^*0 = 01^*0
\]
...
(q0) edge[bend left] node[above] {$1$} (q1)
(q1) edge[bend left] node[below] {$1$} (q0)
(q1) edge[loop above] node {$01^*0$} ();
\end{tikzpicture}
\end{center}
\end{frame}

%------------------------------------------------
\begin{frame}{Step 3: Rip $q_1$}
Now we remove $q_1$. We check for paths that go through $q_1$.
The only path is $q_0 \to q_1 \to q_0$.

\vspace{0.5em}
\textbf{New $q_0 \to q_0$ transition:}
\[
R_{new}(q_0,q_0) = 0 \cup \left( 1 \circ (01^*0)^* \circ 1 \right) = 0 \cup 1(01^*0)^*1
\]
...
\node[state, initial] (qs) {$q_s$};
\node[state, right of=qs] (q0) {$q_0$};
\node[state, accepting, right=3cm of q0] (qa) {$q_a$};

\path 
(qs) edge node[above] {$\eps$} (q0)
(q0) edge node[above] {$\eps$} (qa)
(q0) edge[loop above] node[above=0.1cm] {$0 \cup 1(01^*0)^*1$} ();
\end{tikzpicture}
\end{center}
\end{frame}

%------------------------------------------------
\begin{frame}{Step 4: Rip $q_0$ (Final Regex)}
Finally, we remove $q_0$. The remaining path goes from $q_s \to q_0 \to q_a$.

\vspace{0.5em}
\textbf{Final Expression:}
\[
\eps \circ \left( 0 \cup 1(01^*0)^*1 \right)^* \circ \eps = (0 \cup 1(01^*0)^*1)^*
\]
...
\node[state, initial] (qs) {$q_s$};
\node[state, accepting, right of=qs] (qa) {$q_a$};

\path 
(qs) edge node[above] {$(0 \cup 1(01^*0)^*1)^*$} (qa);
\end{tikzpicture}
\end{center}

\vspace{0.5em}
\small \textbf{Sanity check:} $3$ in binary is $11$, which matches $1(\dots)^*1$. $9$ is $1001$, which matches $1(01^*0)1$. Both are divisible by 3.
\end{frame}
